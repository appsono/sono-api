...
def init_scheduler():
    ...
    scheduler.add_job(run_cleanup_job, trigger=CronTrigger(day_of_week="mon", hour=3, minute=0), id="weekly_cleanup", name="Weekly Data Retention Cleanup", replace_existing=True)

    scheduler.add_job(run_pending_deletions_job, trigger=CronTrigger(hour=4, minute=0), id="daily_pending_deletions", name="Daily Pending Deletions Processing", replace_existing=True)
...
def get_scheduled_jobs():
    ...
    for job in scheduler.get_jobs():
        jobs.append({"id": job.id, "name": job.name, "next_run_time": job.next_run_time.isoformat() if job.next_run_time else None, "trigger": str(job.trigger)})
    return jobs
