...
def create_user(request: Request, user: schemas.UserCreate, db: Session = Depends(get_db), x_password_encrypted: Optional[str] = Header(None)):
    ...
    try:
        user_data = schemas.UserCreate(username=user.username, email=user.email, password=password, display_name=user.display_name)
...
async def search_users(query: str, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_active_user), skip: int = 0, limit: int = 10):
    if not query or len(query.strip()) < 2:
        raise HTTPException(status_code=400, detail="Query must be at least 2 characters")

    users = db.query(models.User).filter(models.User.username.ilike(f"%{query.strip()}%"), models.User.is_active == True).offset(skip).limit(limit).all()
...
def login_for_access_token(request: Request, form_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db), x_password_encrypted: Optional[str] = Header(None)):
    ...
    if not user or not Hasher.verify_password(password, user.hashed_password):
        crud.create_audit_log(db=db, action="login.failed", details=f"Failed login attempt for: {form_data.username}", success=False)
        raise HTTPException(
    ...
    if not user.is_active:
        crud.create_audit_log(db=db, action="login.failed.inactive", user_id=user.id, details=f"Login attempt for deactivated account: {user.username}", success=False)
        raise HTTPException(
    ...
    crud.create_audit_log(db=db, action="login.success", user_id=user.id, details=f"User logged in: {user.username}", success=True)
...
def refresh_token(request: Request, token_request_body: schemas.RefreshTokenRequest, db: Session = Depends(get_db)):
    ...
    crud.create_audit_log(db=db, action="token.refresh", user_id=user.id, details=f"Token refreshed for user: {user.username}", success=True)
...
async def upload_profile_picture(file: UploadFile = File(...), current_user: models.User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    try:
        minio_client.put_object(bucket_name=settings.MINIO_BUCKET_NAME, object_name=unique_filename, data=io.BytesIO(contents), length=len(contents), content_type=file.content_type)
...
@router.post("/logout")
def logout(token_request_body: schemas.RefreshTokenRequest, current_user: models.User = Depends(get_current_active_user), db: Session = Depends(get_db)):
    try:
        payload = jwt.decode(token_request_body.refresh_token, settings.REFRESH_TOKEN_SECRET_KEY, algorithms=[settings.REFRESH_TOKEN_ALGORITHM])
...
def request_account_deletion(deletion_request: schemas.DeletionRequest, current_user: models.User = Depends(get_current_active_user), db: Session = Depends(get_db)):
    ...
    deletion_req = crud.create_deletion_request(db=db, user_id=current_user.id, scheduled_deletion_at=scheduled_deletion, deletion_type=deletion_request.deletion_type, reason=deletion_request.reason)
...
def cancel_account_deletion(current_user: models.User = Depends(get_current_active_user), db: Session = Depends(get_db)):
    ...
    crud.create_audit_log(
        db=db, action="user.deletion_cancelled", user_id=current_user.id, resource_type="user", resource_id=str(current_user.id), details="User cancelled account deletion request", success=True
    )
...
def get_deletion_status(current_user: models.User = Depends(get_current_active_user), db: Session = Depends(get_db)):
    deletion_request = crud.get_deletion_request(db, current_user.id)
...
def delete_account_immediately(request: Request, password: str, deletion_type: str = "soft", current_user: models.User = Depends(get_current_active_user), db: Session = Depends(get_db)):
    from app.core.hashing import Hasher

    if not Hasher.verify_password(password, current_user.hashed_password):
        crud.create_audit_log(db=db, action="user.deletion_failed", user_id=current_user.id, details="Failed account deletion attempt - incorrect password", success=False)
        raise HTTPException(status_code=401, detail="Incorrect password")

    crud.create_audit_log(
        db=db, action=f"user.{deletion_type}_delete", user_id=current_user.id, resource_type="user", resource_id=str(current_user.id), details=f"User account {deletion_type} deleted", success=True
    )
...
def export_user_data(current_user: models.User = Depends(get_current_active_user), db: Session = Depends(get_db)):
    ...
    user_data = {
        ...
        "consents": [
            {"consent_type": consent.consent_type, "consent_version": consent.consent_version, "given_at": consent.given_at.isoformat() if consent.given_at else None, "is_active": consent.is_active}
            for consent in consents
        ],
        "audit_logs": [{"timestamp": log.timestamp.isoformat() if log.timestamp else None, "action": log.action, "resource_type": log.resource_type, "success": log.success} for log in audit_logs],
    }

    crud.create_audit_log(
        db=db, action="user.data_exported", user_id=current_user.id, resource_type="user", resource_id=str(current_user.id), details="User exported personal data (GDPR SAR)", success=True
    )
...
def record_consent(consent: schemas.ConsentCreate, current_user: models.User = Depends(get_current_active_user), db: Session = Depends(get_db)):
    consent_record = crud.create_user_consent(db=db, user_id=current_user.id, consent_type=consent.consent_type, consent_version=consent.consent_version, ip_address=consent.ip_address)

    crud.create_audit_log(db=db, action="consent.given", user_id=current_user.id, details=f"Consent given: {consent.consent_type} v{consent.consent_version}", success=True)

    return {"message": "Consent recorded", "consent_id": consent_record.id}


@router.get("/me/consents")
def get_user_consents_endpoint(current_user: models.User = Depends(get_current_active_user), db: Session = Depends(get_db)):
    consents = crud.get_user_consents(db, current_user.id)
    return consents
...
@limiter.limit("3/hour")
def forgot_password(request: Request, reset_request: schemas.PasswordResetRequest, db: Session = Depends(get_db)):
    ...
    if user:
        ...
        crud.create_password_reset_token(db=db, user_id=user.id, token=reset_token, expires_at=expires_at, ip_address=ip_address)

        email_sent = send_password_reset_email(email=user.email, reset_token=reset_token, username=user.username)

        crud.create_audit_log(db=db, action="password_reset.requested", user_id=user.id, ip_address=ip_address, details=f"Password reset requested for {user.email}", success=email_sent)

        if not email_sent:
            raise HTTPException(status_code=500, detail="Failed to send password reset email. Please try again later.")

    return schemas.PasswordResetResponse(message="If an account with that email exists, we've sent password reset instructions.", success=True)


@router.post("/verify-reset-token", response_model=schemas.PasswordResetResponse)
def verify_reset_token(verify_request: schemas.PasswordResetVerify, db: Session = Depends(get_db)):
...
def reset_password(request: Request, reset_confirm: schemas.PasswordResetConfirm, db: Session = Depends(get_db)):
    reset_token = crud.get_password_reset_token(db, reset_confirm.token)

    if not reset_token:
        crud.create_audit_log(db=db, action="password_reset.failed", details="Invalid reset token used", success=False)
        raise HTTPException(status_code=400, detail="Invalid or expired reset token")
    ...
    crud.create_audit_log(
        db=db, action="password_reset.success", user_id=user.id, ip_address=request.client.host if request.client else None, details=f"Password successfully reset for {user.email}", success=True
    )

    return schemas.PasswordResetResponse(message="Password has been reset successfully. You can now log in with your new password.", success=True)
