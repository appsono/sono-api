...
def get_collection_stats(current_user: models.User = Security(get_current_active_superuser), db: Session = Depends(get_db)):
...
def get_user_collection_stats(user_id: int, current_user: models.User = Depends(get_current_active_superuser), db: Session = Depends(get_db)):
    ...
    return schemas.UserCollectionStats(user_id=user_id, username=user.username, **stats)


@router.delete("/users/{user_id}/collections/all")
def delete_all_user_collections(user_id: int, current_user: models.User = Depends(get_current_active_superuser), db: Session = Depends(get_db)):
    user = crud.get_user(db, user_id)
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
...
def get_collections_summary(current_user: models.User = Depends(get_current_active_superuser), db: Session = Depends(get_db)):
    stats = crud.get_collection_stats(db)

    top_creators = (
        db.query(models.User.id, models.User.username, func.count(models.Collection.id).label("collection_count"))
    ...
    most_collaborative = (
        db.query(models.Collection).options(joinedload(models.Collection.owner)).filter(models.Collection.is_collaborative == True).order_by(models.Collection.created_date.desc()).limit(10).all()
    )

    largest_collections = (
        db.query(models.Collection.id, models.Collection.title, models.Collection.collection_type, models.User.username, func.count(models.CollectionTrack.id).label("track_count"))
        .join(models.User)
        .outerjoin(models.CollectionTrack)
        .group_by(models.Collection.id, models.Collection.title, models.Collection.collection_type, models.User.username)
        .order_by(func.count(models.CollectionTrack.id).desc())
        .limit(10)
        .all()
    )
    ...
    return {
        "stats": stats,
        "top_creators": [{"user_id": creator.id, "username": creator.username, "collection_count": creator.collection_count} for creator in top_creators],
        "most_collaborative": [
            {
                "id": collection.id,
        ...
        "largest_collections": [
            {"id": collection.id, "title": collection.title, "type": collection.collection_type, "owner": collection.username, "track_count": collection.track_count}
...
def get_stats(current_user: models.User = Security(get_current_active_superuser), db: Session = Depends(get_db)):
    ...
    comprehensive_stats = {**user_stats, **collection_stats}

    return comprehensive_stats


@router.get("/users", response_model=List[schemas.User])
def get_all_users(skip: int = Query(0, ge=0), limit: int = Query(100, ge=1, le=1000), current_user: models.User = Depends(get_current_active_superuser), db: Session = Depends(get_db)):
    audit_log(f"User list accessed by admin {current_user.id}")

    users = crud.get_users(db, skip=skip, limit=limit)

    for user in users:
        audio_file_count = db.query(models.AudioFile).filter(models.AudioFile.owner_id == user.id).count()
...
def get_user_details(user_id: int, current_user: models.User = Depends(get_current_active_superuser), db: Session = Depends(get_db)):
    user = crud.get_user(db, user_id)
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
...
def update_user_upload_limit(user_id: int, limit_update: schemas.UserUploadLimitUpdate, current_user: models.User = Depends(get_current_active_superuser), db: Session = Depends(get_db)):
    user = crud.get_user(db, user_id)
    if not user:
        raise HTTPException(status_code=404, detail="User not found")

    if user.is_superuser and user_id != current_user.id:
        raise HTTPException(status_code=400, detail="Cannot modify upload limits for other superusers")

    updated_user = crud.update_user_upload_limit(db, user_id, limit_update.max_audio_uploads)

    if updated_user:
        audit_log(f"Admin {current_user.id} updated user {user_id} upload limit to {limit_update.max_audio_uploads}")
        return updated_user
    else:
...
def get_user_upload_stats(user_id: int, current_user: models.User = Depends(get_current_active_superuser), db: Session = Depends(get_db)):
    user = crud.get_user(db, user_id)
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
...
def get_all_audio_files(skip: int = Query(0, ge=0), limit: int = Query(50, ge=1, le=200), current_user: models.User = Depends(get_current_active_superuser), db: Session = Depends(get_db)):
    audio_files = db.query(models.AudioFile).order_by(models.AudioFile.upload_date.desc()).offset(skip).limit(limit).all()
...
def delete_all_user_audio_files(user_id: int, current_user: models.User = Depends(get_current_active_superuser), db: Session = Depends(get_db)):
    user = crud.get_user(db, user_id)
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
    ...
    audit_log(f"Admin {current_user.id} deleted {deleted_count} audio files for user {user_id} ({failed_count} failed)")

    return {"message": f"Deleted {deleted_count} audio files", "failed": failed_count, "total_processed": len(user_files)}


@router.post("/users/{user_id}/reset-uploads")
def reset_user_uploads(user_id: int, current_user: models.User = Depends(get_current_active_superuser), db: Session = Depends(get_db)):
    user = crud.get_user(db, user_id)
    if not user:
        raise HTTPException(status_code=404, detail="User not found")

    if user.is_superuser and user_id != current_user.id:
        raise HTTPException(status_code=400, detail="Cannot reset uploads for other superusers")
    ...
    audit_log(f"Admin {current_user.id} reset uploads for user {user_id}: deleted {deleted_count} files, reset limit to 20")

    return {"message": f"Reset complete: deleted {deleted_count} files and reset limit to 20", "files_deleted": deleted_count}


@router.post("/users/{user_id}/disable")
def disable_user(user_id: int, current_user: models.User = Depends(get_current_active_superuser), db: Session = Depends(get_db)):
    user = crud.get_user(db, user_id)
    if not user:
        raise HTTPException(status_code=404, detail="User not found")

    if user.is_superuser and user_id != current_user.id:
        raise HTTPException(status_code=400, detail="Cannot disable other superusers")
    ...
    return {"message": f"User {user.username} has been disabled", "user_id": user_id, "username": user.username, "is_active": user.is_active}


@router.post("/users/{user_id}/enable")
def enable_user(user_id: int, current_user: models.User = Depends(get_current_active_superuser), db: Session = Depends(get_db)):
    user = crud.get_user(db, user_id)
    if not user:
        raise HTTPException(status_code=404, detail="User not found")

    if user.is_active:
        raise HTTPException(status_code=400, detail="User is already enabled")
    ...
    return {"message": f"User {user.username} has been enabled", "user_id": user_id, "username": user.username, "is_active": user.is_active}


@router.post("/process-pending-deletions")
def process_pending_deletions_endpoint(current_user: models.User = Depends(get_current_active_superuser), db: Session = Depends(get_db)):
    ...
    return {"message": f"Processed {processed_count} pending deletions", "processed_count": processed_count}


@router.post("/cleanup-profile-pictures")
def cleanup_profile_pictures_endpoint(current_user: models.User = Depends(get_current_active_superuser), db: Session = Depends(get_db)):
    ...
    return {"message": "Profile picture cleanup complete", "deleted_count": result.get("deleted", 0), "error_count": result.get("errors", 0)}


@router.post("/run-all-cleanup-tasks")
def run_all_cleanup_tasks_endpoint(current_user: models.User = Depends(get_current_active_superuser), db: Session = Depends(get_db)):
    from app.tasks.data_retention import cleanup_expired_revoked_tokens, process_pending_deletions, cleanup_old_audit_logs, cleanup_unused_profile_pictures, initialize_default_retention_policies
    ...
    return {"message": "All cleanup tasks completed", "results": results}


@router.get("/scheduled-jobs")
def get_scheduled_jobs_endpoint(current_user: models.User = Depends(get_current_active_superuser), db: Session = Depends(get_db)):
    ...
    return {"message": "Scheduled jobs retrieved", "jobs": jobs, "total_jobs": len(jobs)}
