...
def get_public_announcements(skip: int = Query(0, ge=0), limit: int = Query(20, ge=1, le=100), db: Session = Depends(get_db)):
    ...
    return schemas.AnnouncementListResponse(announcements=announcements, total=total, has_more=has_more)
...
def create_announcement(announcement_data: schemas.AnnouncementCreate, current_user: models.User = Security(get_current_active_superuser), db: Session = Depends(get_db)):
    announcement = crud.create_announcement(db=db, announcement=announcement_data, created_by_id=current_user.id)
...
def get_announcement_admin(announcement_id: int, current_user: models.User = Security(get_current_active_superuser), db: Session = Depends(get_db)):
    announcement = crud.get_announcement(db, announcement_id)
...
def update_announcement(announcement_id: int, announcement_update: schemas.AnnouncementUpdate, current_user: models.User = Security(get_current_active_superuser), db: Session = Depends(get_db)):
    announcement = crud.get_announcement(db, announcement_id)

    if not announcement:
        raise HTTPException(status_code=404, detail="Announcement not found")

    updated_announcement = crud.update_announcement(db=db, announcement_id=announcement_id, announcement_update=announcement_update)
...
def delete_announcement(announcement_id: int, current_user: models.User = Security(get_current_active_superuser), db: Session = Depends(get_db)):
    announcement = crud.get_announcement(db, announcement_id)
    ...
    return {"message": f"Announcement '{announcement_title}' has been deleted", "id": announcement_id}
