...
def get_kworb_db():
    ...
    engine = create_engine(str(settings.KWORB_DATABASE_URL))
...
def get_top_streamed_artists(limit: int = Query(default=100, ge=1, le=1000), offset: int = Query(default=0, ge=0)):
    """Get top streamed artists"""
    db = next(get_kworb_db())
    try:
        result = db.execute(
            text("""
...
def get_monthly_listeners(limit: int = Query(default=100, ge=1, le=1000), offset: int = Query(default=0, ge=0)):
...
def get_top_songs(limit: int = Query(default=100, ge=1, le=1000), offset: int = Query(default=0, ge=0)):
