...
def get_collections(
    ...
    collections = crud.get_collections(db, skip=skip, limit=limit, collection_type=collection_type, public_only=public_only, collaborative_only=collaborative_only)
    ...
    return {"collections": collection_responses, "total": total, "has_more": has_more, "collection_type": collection_type}
...
def get_my_collections(
    ...
    collections = crud.get_collections(db, skip=skip, limit=limit, owner_id=current_user.id, collection_type=collection_type)
...
def get_collaborative_collections(skip: int = Query(0, ge=0), limit: int = Query(20, ge=1, le=100), current_user: models.User = Depends(get_current_active_user), db: Session = Depends(get_db)):
    collaborator_collection_ids = db.query(models.CollectionCollaborator.collection_id).filter(models.CollectionCollaborator.user_id == current_user.id).subquery()

    collections = db.query(models.Collection).filter(models.Collection.id.in_(collaborator_collection_ids)).offset(skip).limit(limit).all()
    ...
    return {"collections": collection_responses, "total": total, "has_more": has_more}


@router.get("/stats", response_model=schemas.UserCollectionStats)
def get_my_collection_stats(current_user: models.User = Depends(get_current_active_user), db: Session = Depends(get_db)):
    stats = crud.get_collection_stats(db, user_id=current_user.id)
    return schemas.UserCollectionStats(user_id=current_user.id, username=current_user.username, **stats)


@router.post("/", response_model=schemas.Collection)
def create_collection(collection: schemas.CollectionCreate, current_user: models.User = Depends(get_current_active_user), db: Session = Depends(get_db)):
...
def get_collection(collection_id: int, current_user: Optional[models.User] = Depends(get_optional_current_user), db: Session = Depends(get_db)):
...
def update_collection(collection_id: int, collection_update: schemas.CollectionUpdate, current_user: models.User = Depends(get_current_active_user), db: Session = Depends(get_db)):
    collection = crud.get_collection(db, collection_id)
    if not collection:
        raise HTTPException(status_code=404, detail="Collection not found")
...
def delete_collection(collection_id: int, current_user: models.User = Depends(get_current_active_user), db: Session = Depends(get_db)):
    collection = crud.get_collection(db, collection_id)
    if not collection:
        raise HTTPException(status_code=404, detail="Collection not found")
...
def download_collection(collection_id: int, current_user: Optional[models.User] = Depends(get_optional_current_user), db: Session = Depends(get_db)):
    collection = crud.get_collection(db, collection_id)
    if not collection:
        raise HTTPException(status_code=404, detail="Collection not found")
    ...
    try:
        ...
        return StreamingResponse(zip_buffer, media_type="application/zip", headers={"Content-Disposition": f"attachment; filename=\"{ascii_filename}\"; filename*=UTF-8''{utf8_filename}"})
...
async def upload_collection_cover_art(collection_id: int, file: UploadFile = File(...), current_user: models.User = Depends(get_current_active_user), db: Session = Depends(get_db)):
    collection = crud.get_collection(db, collection_id)
    if not collection:
        raise HTTPException(status_code=404, detail="Collection not found")
    ...
    try:
        ...
        minio_client.put_object(bucket_name="cover-art", object_name=stored_filename, data=io.BytesIO(contents), length=file_size, content_type=file.content_type)
...
def add_track_to_collection(collection_id: int, track_data: schemas.CollectionTrackCreate, current_user: models.User = Depends(get_current_active_user), db: Session = Depends(get_db)):
    collection = crud.get_collection(db, collection_id)
    if not collection:
        raise HTTPException(status_code=404, detail="Collection not found")
...
def remove_track_from_collection(collection_id: int, track_id: int, current_user: models.User = Depends(get_current_active_user), db: Session = Depends(get_db)):
    collection = crud.get_collection(db, collection_id)
    if not collection:
        raise HTTPException(status_code=404, detail="Collection not found")
...
def reorder_collection_track(
    collection_id: int, track_id: int, reorder_data: schemas.CollectionTrackReorder, current_user: models.User = Depends(get_current_active_user), db: Session = Depends(get_db)
):
    collection = crud.get_collection(db, collection_id)
    if not collection:
        raise HTTPException(status_code=404, detail="Collection not found")
...
def bulk_add_tracks_to_collection(collection_id: int, bulk_data: schemas.BulkAddTracks, current_user: models.User = Depends(get_current_active_user), db: Session = Depends(get_db)):
    collection = crud.get_collection(db, collection_id)
    if not collection:
        raise HTTPException(status_code=404, detail="Collection not found")
...
def bulk_reorder_collection_tracks(collection_id: int, bulk_data: schemas.BulkReorderTracks, current_user: models.User = Depends(get_current_active_user), db: Session = Depends(get_db)):
    ...
    for reorder_item in bulk_data.track_orders:
        track = crud.reorder_collection_track(db, collection_id, reorder_item["track_id"], reorder_item["new_order"])
        if track:
            success_count += 1

    return {"message": f"Successfully reordered {success_count} tracks", "processed": len(bulk_data.track_orders)}
...
def add_collection_collaborator(
    collection_id: int, collaborator_data: schemas.CollectionCollaboratorCreate, current_user: models.User = Depends(get_current_active_user), db: Session = Depends(get_db)
):
    collection = crud.get_collection(db, collection_id)
    if not collection:
        raise HTTPException(status_code=404, detail="Collection not found")
...
def update_collection_collaborator(
    collection_id: int, user_id: int, collaborator_update: schemas.CollectionCollaboratorUpdate, current_user: models.User = Depends(get_current_active_user), db: Session = Depends(get_db)
):
    collection = crud.get_collection(db, collection_id)
    if not collection:
        raise HTTPException(status_code=404, detail="Collection not found")
...
def remove_collection_collaborator(collection_id: int, user_id: int, current_user: models.User = Depends(get_current_active_user), db: Session = Depends(get_db)):
    collection = crud.get_collection(db, collection_id)
    if not collection:
        raise HTTPException(status_code=404, detail="Collection not found")

    if collection.owner_id != current_user.id and user_id != current_user.id and not current_user.is_superuser:
        raise HTTPException(status_code=403, detail="You can only remove yourself or (as owner) remove other collaborators")
...
def get_albums(skip: int = Query(0, ge=0), limit: int = Query(20, ge=1, le=100), public_only: bool = Query(False), db: Session = Depends(get_db)):
...
def get_playlists(skip: int = Query(0, ge=0), limit: int = Query(20, ge=1, le=100), public_only: bool = Query(False), db: Session = Depends(get_db)):
...
def get_compilations(skip: int = Query(0, ge=0), limit: int = Query(20, ge=1, le=100), public_only: bool = Query(False), db: Session = Depends(get_db)):
...
def admin_delete_collection(collection_id: int, current_user: models.User = Depends(get_current_active_superuser), db: Session = Depends(get_db)):
    collection = crud.get_collection(db, collection_id)
    if not collection:
        raise HTTPException(status_code=404, detail="Collection not found")
