...
def get_maintenance_status():
    ...
    return MaintenanceStatus(enabled=maintenance_state.is_enabled(), message=maintenance_state.get_message())


@router.post("/toggle", response_model=MaintenanceStatus)
def toggle_maintenance(toggle: MaintenanceToggle, current_user: models.User = Security(get_current_active_superuser)):
    ...
    if toggle.enabled:
        ...
        return MaintenanceStatus(enabled=True, message=maintenance_state.get_message())
    else:
        maintenance_state.disable()
        return MaintenanceStatus(enabled=False, message=maintenance_state.get_message())


@router.post("/enable", response_model=MaintenanceStatus)
def enable_maintenance(message: Optional[str] = None, current_user: models.User = Security(get_current_active_superuser)):
...
def disable_maintenance(current_user: models.User = Security(get_current_active_superuser)):
