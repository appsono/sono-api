...
async def upload_audio_file(
    ...
    if not can_upload:
        raise HTTPException(
            status_code=403, 
            detail=f"Upload limit reached. You have {current_count} out of {max_allowed} audio files."
        )
    ...
    if file.content_type not in ALLOWED_AUDIO_TYPES:
        ...
        raise HTTPException(
            status_code=400, 
            detail=f"Unsupported audio format. Allowed formats: {allowed_types_str}"
        )
...
def update_audio_file(
    file_id: int,
    file_update: schemas.AudioFileUpdate,
    current_user: models.User = Depends(get_current_active_user),
    db: Session = Depends(get_db)
):
    audio_file = crud.get_audio_file(db, file_id)
