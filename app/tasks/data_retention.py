...
def cleanup_unused_profile_pictures(db: Session):
    ...
    try:
        ...
        users_with_profile_pics = db.query(models.User.profile_picture_url).filter(models.User.profile_picture_url.isnot(None)).all()
...
def initialize_default_retention_policies(db: Session):
    ...
    for policy in default_policies:
        try:
            crud.create_retention_policy(db=db, data_type=policy["data_type"], retention_days=policy["retention_days"], description=policy["description"])
